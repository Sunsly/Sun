...
_NSConcreteGlobalBlock 全局静态   (数据区)

_NSConcreteStackBlock 保存在栈中，出函数作用域就销毁（栈区）

_NSConcreteMallocBlock 保存在堆中，retainCount == 0销毁（堆区）
...
43 可以说几个重构的技巧么？你觉得重构适合什么时候来做？

*********数据结构&算法题
44.链表和数组的区别是什么？插入和查询的时间复杂度分别是多少？

45.哈希表是如何实现的？如何解决地址冲突？
     
46.排序题：冒泡排序，选择排序，插入排序，快速排序（二路，三路）能写出那些？
     
47.链表题：如何检测链表中是否有环？如何删除链表中等于某个值的所有节点？
     
48.数组题：如何在有序数组中找出和等于给定值的两个元素？如何合并两个有序的数组之后保持有序？
     
49.二叉树题：如何反转二叉树？如何验证两个二叉树是完全相等的？
     
...
sdwebimage缓存图片怎么实现的，在内存中有缓存吗? 搜房网
...
————————————————
版权声明：本文为CSDN博主「技术熊」的原创文章，遵循 CC 4.0 BY-SA 版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/u013513053/article/details/86521041

你单例怎么理解怎么用的? 搜房网

来说说oc这门语言吧，和c c++比，你觉得哪个效率高，高到哪里了? 搜房网
...
https://blog.csdn.net/u013513053/article/details/86521041
...
// 组件化的优缺点
优点：组件可独立运行，提高的代码的复用性，组件化的颗粒度越细，可复用度就越高。
...
查看Main()函数后的花费时间 方案一：定时抓取主线程方法的调用堆栈，计算一段时间里的方法耗时。（Xcode中的Time Profiler就是使用的这种的方法）
方案二：对objc_msgSend方法进行hook，来得到所有方法的耗时。

TimeProfiler的基本使用（一定要用真机调试，因为模拟器使用的是电脑的CPU，并不能检测出真正的性能问题。）
...
   链表开发于1955-56，由当时所属于兰德公司（英语：RAND Corporation）的艾伦纽维尔（AllenNewell），克里夫肖（Cliff Shaw）和赫伯特西蒙（Herbert Simon）在他们编写的信息处理语言（IPL）中做为原始数据类型所编写。IPL被作者们用来开发几种早期的人工智能程序，包括逻辑推理机，通用问题解算器和一个计算机象棋程序。
